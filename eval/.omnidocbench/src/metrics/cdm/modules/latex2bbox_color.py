...
def convert_pdf2img(pdf_filename, png_filename, timeout_sec=30, all_pages=False):
    ...
    if all_pages:
        cmd = [magick_bin, "-density", "200", "-quality", "100", pdf_filename, "-append", tmp_png]
    else:
        pdf_spec = f"{pdf_filename}[0]"
        cmd = [magick_bin, "-density", "200", "-quality", "100", pdf_spec, tmp_png]
...
def collapse_cjk_text_groups(tokens):
    ...
    while i < len(tokens):
        ...
        i = j + 1
    return out
def _safe_temp_prefix(output_path: str, basename: str) -> str:
    from pathlib import Path
    safe_output = re.sub(r"[^0-9A-Za-z_.-]+", "_", Path(output_path).name).strip("_.")
    if not safe_output:
        safe_output = "cdm"
    return f"{safe_output}_{basename}"
def latex2bbox_color(input_arg):
    ...
    try:
        ...
        logging.info(log)
        return
    pre_name = _safe_temp_prefix(output_path, basename)
    ...
    for ps in paper_candidates:
        ...
        pdflatex_bin = resolve_tex_binary("pdflatex")
        tex_arg = os.path.abspath(tex_filename).replace(os.sep, "/")
        output_dir_arg = os.path.abspath(temp_dir).replace(os.sep, "/")
        run_cmd(
            [
                pdflatex_bin,
